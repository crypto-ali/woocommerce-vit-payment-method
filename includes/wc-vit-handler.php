...
class WC_VIT_Handler {
	...
	public static function register_schedulers() {
		...
		if ( ! wp_next_scheduled('wc_vit_update_rates')) {
			wp_schedule_event(time(), 'hourly', 'wc_vit_update_rates');
		}

		if ( ! wp_next_scheduled('wc_vit_update_orders')) {
			wp_schedule_event(time(), '2min', 'wc_vit_update_orders');
		}
		
		if ( ! wp_next_scheduled('wc_vit_send_pending_payment_emails')) {
			wp_schedule_event(time(), '5min', 'wc_vit_send_pending_payment_emails');
		}		

		if (empty(get_option('wc_vit_rates'))) {
			self::update_rates();
		}

		add_action('wc_vit_update_rates', array($instance, 'update_rates'));
		add_action('wc_vit_update_orders', array($instance, 'update_orders'));
		add_action('wc_vit_send_pending_payment_emails', array($instance, 'send_pending_payment_emails'));
	}
	...
	public static function update_rates() {
		...
		$ecb_response = wp_remote_get('https://api.exchangeratesapi.io/latest?base=USD');

		if (is_array($ecb_response)) {
			$tickers = json_decode(wp_remote_retrieve_body($ecb_response), true);
			...
			if (isset($tickers['rates']) && $tickers['rates']) {
				foreach ($tickers['rates'] as $to_currency_symbol => $to_currency_value) {
					...
					if (isset($rates['VIT_USD'])) {
						$rates["VIT_{$to_currency_symbol}"] = $rates['VIT_USD'] * $to_currency_value;
					}
		...
		update_option('wc_vit_rates', $rates);
	}
	...
	public static function update_orders() {
		...
		$query1 = new WP_Query(array(
			...
			'meta_query' => array(
				...
				array(
					'key' => '_wc_vit_transaction_transfer',
					'compare' => 'NOT EXISTS',
				),
			),
		...
		$query2 = new WP_Query(array(
			...
			'meta_query' => array(
				...
				array(
					'key' => '_wc_vit_last_searched_for_transaction',
					'compare' => 'NOT EXISTS',
				),
			),
	...
	public static function update_order($order) {
		...
		if ( ! empty(get_post_meta($order->get_id(), '_wc_vit_transaction_transfer', true))) {
			return;
		}

		$transfer = WC_VIT_Transaction_Transfer::get($order);
		
		if ($transfer != null) {
			// Mark payment as completed
			$order->payment_complete();
		
			$payee = wc_order_get_vit_payee($order->get_id());

			// Add intuitive order note
			$order->add_order_note(
				sprintf(
					__('vit payment <strong>Received</strong><br />Time: %s<br />Memo: %s<br />Payee: %s<br />%s', 'wc-vit'), 
					$transfer['time'], 
					$transfer['memo'], 
					$payee,
			...
			update_post_meta($order->get_id(), '_wc_vit_status', 'paid');
			update_post_meta($order->get_id(), '_wc_vit_transaction_transfer', $transfer);
		}		
	}
	...
	public static function send_pending_payment_emails() {
		...
		$orders = get_posts(array(
			...
			'meta_query' => array(
				...
				array(
					'key' => '_wc_vit_pending_payment_email_sent',
					'compare' => 'NOT EXISTS',
				),
			),
		));
	...
	public static function send_pending_payment_email($order) {
		...
		if ($order->get_payment_method() != 'wc_vit') return;
		...
		update_post_meta($order->get_id(), '_wc_vit_pending_payment_email_sent', 'true');
	}
				...
				<tbody>
					<tr>
						...
						<td class="td" style="text-align: left; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif; word-wrap: break-word; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . 
							wc_order_get_vit_payee($order_id) . '</td>
					</tr>
					<tr>
						<th class="td" style="text-align: left; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif; word-wrap: break-word; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . 
							__('Memo', 'wc-vit') . '</th>
						<td class="td" style="text-align: left; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif; word-wrap: break-word; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . 
							wc_order_get_vit_memo($order_id) . '</td>
					</tr>
					<tr>
						<th class="td" style="text-align: left; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif; word-wrap: break-word; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . 
							__('Amount', 'wc-vit') . '</th>
						<td class="td" style="text-align: left; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif; word-wrap: break-word; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . 
							wc_order_get_vit_amount($order_id) . '</td>
					</tr>
					<tr>
						<th class="td" style="text-align: left; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif; word-wrap: break-word; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . 
							__('Currency', 'wc-vit') . '</th>
						<td class="td" style="text-align: left; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif; word-wrap: break-word; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . 
							wc_order_get_vit_amount_currency($order_id) . '</td>
					</tr>
				</tbody>
				</table>
			</div>
